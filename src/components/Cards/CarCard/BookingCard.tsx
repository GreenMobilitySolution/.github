...
const BookingCarCard: React.FC<CarCardProps> = ({ car, selectedCar, selectedTimeSlot, onTimeSlotSelect, onSeatSelect, selectedSeats, onChooseCar }) => {
  const [localSelectedTimeSlot, setLocalSelectedTimeSlot] = useState<string>("");
          ...
          {car.availableTimeSlots.map((slot) => (
            <div key={slot.time} className="flex items-center mb-2">
              <input
                ...
                className="mr-2"
              />
              <label className={`p-2 border rounded-md ${localSelectedTimeSlot === slot.time ? "bg-blue-500 text-white" : "bg-gray-200"}`}>
        ...
        {localSelectedTimeSlot && (
          <div className="mt-4">
            ...
            <input
              ...
              min="1"
              max={car.availableTimeSlots.find(slot => slot.time === localSelectedTimeSlot)?.seats || 1}
