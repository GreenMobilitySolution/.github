...
export function BusStopSection(props: BusStopProp) {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filteredBusStops = filteredBusStops.filter(busStop => busStop.name.toLowerCase().includes(searchTerm.toLowerCase()));
    }
              ...
              {viewBusStops.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {viewBusStops.map((busStop) => (
                    <BusStopCard key={busStop.id} busStop={busStop} route={{
                ...
                <div className="text-center py-20">
                  <p className="text-gray-500">No bus stops found for the search query.</p>
                </div>
              )}
            </>
          )}

          {!loading && busStopList.length > viewItems && viewBusStops.length > 0 && (
